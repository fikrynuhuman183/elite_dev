...
class PaymentService {
    ...
    public function getTotalPaidAmount($invoiceNumber) {
        $stmt = $this->conn->prepare("
            ...
            WHERE invoice_number = ? AND payment_type = 'payment'
        ");
        $stmt->bind_param("s", $invoiceNumber);
        $stmt->execute();
        $result = $stmt->get_result()->fetch_assoc();
    ...
    private function insertPaymentRecord($data) {
        $stmt = $this->conn->prepare("
    ...
    public function updateInvoicePaymentSummary($invoiceNumber) {
        $stmt = $this->conn->prepare("
            SELECT 
                ...
                ROUND(COALESCE(SUM(CASE WHEN pr.payment_type = 'payment' THEN ABS(pr.payment_amount) ELSE 0 END), 0), 3) as total_paid,
